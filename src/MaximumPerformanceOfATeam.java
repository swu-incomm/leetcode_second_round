...
public class MaximumPerformanceOfATeam {
    class Engineer {
        ...
        public Engineer (int eff, int sp) {
            this.eff = eff;
            this.sp= sp;
        }
        public int getEff() {
    ...
    public int maxPerformance(int n, int[] speed, int[] efficiency, int k) {
        ...
        Collections.sort(engineers, (a, b)->b.getEff() - a.getEff());
        PriorityQueue<Integer> pq = new PriorityQueue<>();
        long hireSpeed = 0;
        long res = 0;
        for(Engineer engineer: engineers) {
            int curEff = engineer.getEff();
            int curSp = engineer.getSp();
            if(pq.size() == k) {
                hireSpeed-=pq.poll();
            }
            hireSpeed += curSp;
            pq.offer(curSp);
            res = Math.max(res, hireSpeed * curEff);
        }
        return (int)(res%modular);
    }
}