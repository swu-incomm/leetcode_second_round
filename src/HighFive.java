 ...
 *
 * Return the answer as an array of pairs result, where result[j] = [IDj, topFiveAveragej] represents the
 * student with IDj and their top five average. Sort result by IDj in increasing order.
 *
...
public class HighFive {
    public int[][] highFive(int[][] items) {
        ...
        for(Map.Entry entry: map.entrySet()) {
            PriorityQueue temp = (PriorityQueue) entry.getValue();
            int average = 0;
            for(int i = 0; i<5; i++) {
                average += (int)temp.poll();
            }
            average/=5;
            res[j][0] = (int)entry.getKey();
            res[j++][1] = average;
        }
