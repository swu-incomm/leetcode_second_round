 ...
 * the rightmost stone from the row and receive points equal to the sum of the remaining stones' values in the row. The winner
...
public class StoneGameVII {
    ...
    public int stoneGameVII(int[] stones) {
        ...
        return backtrack(0, n-1, sum, stones);
    }

    //basically Alice and Bob take same strategy to reach the maximum score
    public int backtrack(int left, int right, int sum, int [] stones) {
        if(left == right) {
            return 0;
        }
        if(right - left ==1) {
        ...
        int ans = Math.max(sum - stones[left] - backtrack(left + 1, right, sum-stones[left], stones),
                sum - stones[right] - backtrack(left, right-1, sum-stones[right], stones));
        dp[left][right] = ans;
        return ans;
    }
}
