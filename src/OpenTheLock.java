 ...
 * We can think of this problem as a shortest path problem on a graph: there are `10000` nodes
 * (strings `'0000'` to `'9999'`), and there is an edge between two nodes if they differ in one digit,
    ...
    public int openLock(String[] deadends, String target) {
        ...
        while(!queue.isEmpty()) {
            ...
            for(int i=0; i<size; i++) {
                ...
                if(!deadendsSet.contains(temp)) {
                    visited.add(temp);
                    for(int j = 0; j<4; j++) {
                        //for each character +1/-1
                        for(int k = -1; k<=1; k+=2) {
                            //we can't use string.replace here, because it only replace at the first occurance of the char
                            char tempChar = temp.charAt(j);
                            int tempCharInt = (tempChar - '0' + k + 10) %10;
                            String nextString = temp.substring(0, j) + tempCharInt + temp.substring(j+1);
